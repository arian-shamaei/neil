...
SELFCHECK_FAILS=0
SELFCHECK_PASS=0
if grep -q "FAIL" /tmp/openclaw-e2e-selfcheck.log 2>/dev/null; then
    SELFCHECK_FAILS=$(grep -c "FAIL" /tmp/openclaw-e2e-selfcheck.log)
fi
if grep -q "ALL CHECKS PASSED" /tmp/openclaw-e2e-selfcheck.log 2>/dev/null; then
    SELFCHECK_PASS=1
fi
